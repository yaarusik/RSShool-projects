...
class View {
  ...
  static changeRibbonColor(element: Element, change: string): void {
    switch (change) {
      case 'add': {
        element.children[ballIndicatorElement]?.classList.add('card__active');
        break;
      }
      default: {
        element.children[ballIndicatorElement]?.classList.remove('card__active');
      }
    }
  }
